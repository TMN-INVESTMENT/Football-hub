...
<input type="number" id="multiStakeAmount" class="form-input" 
       ...
       oninput="bettingSystem.calculateMultiProfit()">
                ...
                <button class="vip-bet-btn" style="margin-top: 1.5rem;" onclick="bettingSystem.placeMultiBet()" id="placeMultiBetBtn">
    ...
    if (announcements.length === 0) {
        ...
        announcements.forEach(ann => {
            ...
            if (type === 'text') {
                ...
                // Build team logos with fallback
                const homeLogoHtml = homeLogo
                    ? `<img src="${homeLogo}" alt="${homeTeam}" onerror="this.style.display='none';this.parentElement.innerHTML+='<div class=\\'no-logo\\'>${getInitial(homeTeam)}</div>';">`
                    : `<div class="no-logo">${getInitial(homeTeam)}</div>`;
                const awayLogoHtml = awayLogo
                    ? `<img src="${awayLogo}" alt="${awayTeam}" onerror="this.style.display='none';this.parentElement.innerHTML+='<div class=\\'no-logo\\'>${getInitial(awayTeam)}</div>';">`
...
function renderUserCard(data) {
    ...
    if (type === 'other' && data.mediaUrl) {
        ...
        const homeLogoHtml = homeLogo ?
            `<img src="${homeLogo}" alt="${homeTeam}" onerror="this.style.display='none';this.parentElement.innerHTML+='<div class=\\'no-logo-small\\'>${getInitial(homeTeam)}</div>';">` :
            `<div class="no-logo-small">${getInitial(homeTeam)}</div>`;
        
        const awayLogoHtml = awayLogo ?
            `<img src="${awayLogo}" alt="${awayTeam}" onerror="this.style.display='none';this.parentElement.innerHTML+='<div class=\\'no-logo-small\\'>${getInitial(awayTeam)}</div>';">` :
